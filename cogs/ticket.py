...
class OpenTicketModal(Modal, title="Appeals Ticket"):
    # Matching your screenshot fields
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 3. Create or Update Previous Ticket History in User's Nickname
        # NOTE: If you want this to be permanent across bot restarts, you should use a JSON or SQLite file.
        # For now, we'll store it in their nickname.
        previous_tickets = []
        ...
        if not interaction.user.nick:
            ...
            new_nick = interaction.user.nick.split(" |")[0] # Clean up old metadata
        ...
        try:
            ...
            pass # Ignore if bot doesn't have nickname perms

        # 4. Create Private Category
        category_overwrites = {
        ...
        for role in self.support_roles:
            for member in role.members:
                if member.id != interaction.user.id: # Don't assign to the opener
                    assigned_staff = member
                    break
            if assigned_staff:
                break

        # 7. Create 3x Embeds (Panel, Profile, History)
        # Embed 1: Ticket Controls & Info
        ...
        # 8. Send Initial Message and Pinned Message
        ...
        # 9. Send Auto-Assign / Question Answers Message
        answers_embed = discord.Embed(
            color=discord.Color.from_rgb(46, 204, 113) # Green like the screenshot
        )
        ...
        # 10. Pin the question/answers message (like the screenshot)
        try:
...
# ============================================
# 3. TICKET CONTROL VIEW (WITH AUTO-ASSIGN MENTIONS)
# ============================================
